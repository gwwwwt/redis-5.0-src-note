...
<img src="./dict.png" alt="字典结构" style="zoom:80%;" />
...
<img src="./dict_tmp1.png" alt="dict_tmp1" style="zoom:50%;" />
...
dictEntry *dictAddRaw(dict *d, void *key, dictEntry **existing)
{
    ...
    // _dictKeyIndex函数, 查找键, 找到则直接返回 -1, 并把老节点存入existing字段;
...
若在字典中已存在key这个键值, 则返回 -1; 并且在这种情况下, 如果existing不为NULL, 则将它指向对应的dictEntry; 
*/
...
/*
扩容的主要流程: 
1. 申请一块内存, 初次申请时默认容量大小为4个dictEntry; 非初次申请时, 申请内存大小为当前hash表容量的一倍; 
2. 新申请的内存地址赋值为ht[1],并把字典的rehashidx标识由 -1改为0,表示之后需要进行rehash操作; 
*/
